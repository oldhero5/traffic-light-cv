...
from src.localization.localizer import CameraCalibration, ObjectLocalizer
...
class TestLocalizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.calibration = CameraCalibration(
            image_width=1920, image_height=1080, fov_horizontal=60.0, fov_vertical=35.0
        )
        ...
        self.detections = [
            ...
            Detection(
                class_id=4, class_name="traffic_camera", confidence=0.85, bbox=(500, 300, 600, 400)
            ),
        ]
    ...
    def test_calculate_world_position(self):
        ...
        position = self.localizer._calculate_world_position(distance, bearing, self.gps_data)
        ...
        position_zero = self.localizer._calculate_world_position(0.0, bearing, self.gps_data)

        # Should be very close to original position
        self.assertAlmostEqual(position_zero["latitude"], self.gps_data["latitude"], places=6)
        self.assertAlmostEqual(position_zero["longitude"], self.gps_data["longitude"], places=6)
