...
class MappedObject:
    ...
    def update_position(self, lat: float, lon: float, alt: Optional[float] = None) -> None:
        """
...
class Mapper:
    ...
    def update(self, localized_objects: List[Dict], frame_number: int) -> List[MappedObject]:
        ...
        for obj in localized_objects:
            ...
            if existing_obj:
                ...
                existing_obj.confidence = (
                    existing_obj.confidence * 0.8 + detection.confidence * 0.2
                )
            else:
                # Create new object
    ...
    def get_objects_by_class(self, class_name: str) -> List[MappedObject]:
        ...
        """
        return [
            obj for obj in self.mapped_objects.values()
            if class_name in obj.class_name
        ]
    ...
    def _find_nearby_object(
        ...
        for idx in nearby_indices:
            for obj in self.mapped_objects.values():
                if (
                    obj.object_id == f"idx_{idx}" and
                    class_name in obj.class_name
                ):
    ...
    def _save(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Saved {len(self.mapped_objects)} objects to {self.persistence_file}")
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(self.mapped_objects)} objects from {self.persistence_file}")
        except Exception as e:
