...
def augment_dataset(
    ...
    # Get image files
    image_files = [f for f in os.listdir(images_input_dir) if f.lower().endswith((".jpg", ".jpeg", ".png"))]
    ...
    for img_file in tqdm(image_files, desc="Augmenting images"):
        ...
        for i in range(num_augmentations):
            ...
            aug_img_file = f"{os.path.splitext(img_file)[0]}_aug_{i+1}{os.path.splitext(img_file)[1]}"
    ...
    logger.info(f"Dataset augmentation complete. Augmented {len(image_files)} images with {num_augmentations} variations each.")